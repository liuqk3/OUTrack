...
class Logger(object):
    def __init__(self, opt):
        ...
        if USE_TENSORBOARD:
            ...
            if not os.path.exists(log_dir):
                os.mkdir(log_dir)
        self.log = open(os.path.join(log_dir, 'log.txt'), 'a') # 'w')
        self.train_log = open(os.path.join(log_dir, 'train_log.txt'), 'a') # 'w')
        try:
    ...
    def write(self, txt):
        ...
        if '\n' in txt:
            self.start_line = True
            self.log.flush()
    def write_training_log(self, txt):
        time_str = time.strftime('%Y-%m-%d-%H-%M')
        self.train_log.write('{}: {}'.format(time_str, txt+ '\n'))
